...
- **Egress defaults to unrestricted** (open — the box can reach any host; `cos` prints a one-line UNRESTRICTED notice). Use `-p`/`-e` to **restrict** outbound to an **exact** set when the job needs isolating (e.g. an untrusted build): `python-uv` (astral.sh, pypi, pythonhosted), `rust-cargo` (crates.io ×3, rust-lang, **cdn.pyke.io** — ort-sys/ONNX), `npm`, `github`. Compose presets/domains to combine, or `-E` to keep it explicitly unrestricted. The default does **not** restrict anything — isolating an untrusted build means opting in with `-e <host>` or the matching preset.
