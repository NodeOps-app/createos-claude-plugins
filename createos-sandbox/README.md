...
```bash
claude --plugin-dir /path/to/createos/createos-sandbox
/reload-plugins      # after editing plugin files
```

**Persistent (local marketplace):**
```
/plugin marketplace add /path/to/createos
...
| `/createos-sandbox:shell [-s shape] [-r rootfs] [-e dom\|-p preset\|-E]` | instant throwaway interactive Linux (destroyed on exit; run via `!cos shell`; egress unrestricted by default like `offload`, `-e`/`-p` restrict) |
| `/createos-sandbox:up [-s shape] [-r rootfs] [-n name] [-e dom\|-p preset\|-E]` | create/reuse the per-repo project box (egress unrestricted by default like `offload`, `-e`/`-p` restrict) |
...
- **Egress defaults to unrestricted** (the box can reach any host — `cos` prints a one-line UNRESTRICTED notice). `-p`/`-e` **restrict** outbound to an **exact** set instead: `-p python-uv` (astral.sh, pypi, pythonhosted), `-p rust-cargo` (crates.io ×3, rust-lang, **cdn.pyke.io** ← ort-sys/ONNX), `-p npm`, `-p github`. Compose them, add stragglers with `-e <host>`, or `-E` to keep it explicitly unrestricted.
