...
Bring up (or reuse) the project sandbox. State is tracked per git root, so repeated runs reuse the same warm box. Supports the same egress allowlisting as `offload`: `-e <domain>` (repeatable), `-p <preset>` (e.g. `python-uv`, `rust-cargo`, `npm`, `github`), or `-E` for unrestricted.
